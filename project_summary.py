...
check =True )
from jinja2 import Template
# For HTML templating via Jinja2from jinja2 import Template
# For GUI (if requested)try :
import tkinter as tk
from tkinter import filedialog,
messagebox
except ImportError :
tk =None
# Ensure stdout uses UTF-8 (helpful on Windows)sys .stdout .reconfigure (encoding ='utf-8')
# ====================================# User-Requested Settings (defaults)# ====================================DRY_RUN_DEFAULT =False
VERBOSE_DEFAULT =False
HIGH_COMPLEXITY_THRESHOLD_DEFAULT =10
# ====================================# Required Python Libraries for scanning# ====================================REQUIRED_LIBRARIES =[
    "radon",
    "bandit",
    "flake8",
    "sqlparse",
    ...
    "safety",
    "markdown",
    # For converting Markdown to HTML"jsonschema",
    # For validating manifest.json schema"jinja2",
    # For HTML templating# pip-audit can also be installed: "pip_audit"
]
# Folders & files to excludeEXCLUDED_FOLDERS =[
    "node_modules",
    "dist",
    "build",
    "venv",
...
EXCLUDED_FILES =[
    "python_project_tree.py",
    "project_summary.py",
    "project_summary.html"
]
# Known sensitive patternsSENSITIVE_PATTERNS =[
    r"API_KEY\s*=\s*[\"'][A-Za-z0-9_\-]{20,}[\"']",
    r"SECRET_KEY\s*=\s*[\"'][A-Za-z0-9_\-]{20,}[\"']",
    r"PASSWORD\s*=\s*[\"'].*[\"']",
    r"DATABASE_URL\s*=\s*[\"']postgres://.*[\"']"
]
# Output filenamesSUMMARY_FILE ="project_summary.md"
...
# Added "CSS" category for plain CSS files.self .summary ={
    "Backend": [
    ],
    "Frontend": [
    ],
...
result [
    ...
    l .strip ()for l in js_cmd .stdout .splitlines ()if l .strip ()
]
except Exception as e :
if self .verbose :
...
return results
def detect_all_unused_dependencies (self,
languages_found : set )->dict :
"""
...
all_unused [
    "Go"
]=self .run_unused_deps_for_go ()
if "Rust"in languages_found :
all_unused [
    "Rust"
]=self .run_unused_deps_for_rust ()
if "PHP"in languages_found :
all_unused [
    "PHP"
]=self .run_unused_deps_for_php ()
if "C#"in languages_found :
all_unused [
    "C#"
]=self .run_unused_deps_for_csharp ()
if "Java"in languages_found :
all_unused [
    "Java"
]=self .run_unused_deps_for_java ()
return all_unused
...
for line in lines :
line_strip =line .strip ()
if re .search (r"(SELECT|INSERT|UPDATE|DELETE)\s+",
line_strip,
re .IGNORECASE ):
inside_query =True
...
inside_query =False
return queries
def extract_tailwind_classes (self,
line : str )->list :
"""
...
match =re .findall (r'className=["\'](.*?)["\']',
line )
for m in match :
classes .extend (m .split ())
return classes
def extract_bootstrap_classes (self,
line : str )->list :
"""
...
match =re .findall (r'(?:class|className)\s*=\s*["\']([^"\']+)["\']',
line )
for m in match :
for c in m .split ():
if re .search (r'^(container|row|col-|btn|table|active|disabled|danger|warning|success|info|navbar|dropdown)',
c,
re .IGNORECASE ):
classes .append (c )
return classes
...
func_pat =re .compile (r'function\s+([\w$]+)\s*\((.*?)\)\s*\{?',
re .MULTILINE )
arrow_pat =re .compile (r'(?:const|let|var)\s+([\w$]+)\s*=\s*(?:async\s+)?\(?([^\)]*?)\)?\s*=>',
re .DOTALL )
func_expr_pat =re .compile (r'(?:const|let|var)\s+([\w$]+)\s*=\s*function\s*\((.*?)\)\s*\{?',
re .MULTILINE )
class_method_pat =re .compile (r'^\s*([\w$]+)\s*\((.*?)\)\s*\{',
re .MULTILINE )
...
return js_funcs
def extract_javascript_invocations (self,
lines : list,
defined_funcs : list )->list :
"""
...
security_data ={
    ...
    "eslint": self .run_eslint ()or [
        "✅ ESLint ran and found no issues."
    ],
    "stylelint": self .run_stylelint ()or [
        "✅ Stylelint ran and found no issues."
    ],
    "webext": self .run_web_ext_lint ()or [
        "✅ web-ext lint ran and found no issues."
    ],
    "pip_audit": self .run_pip_audit ()or [
        "✅ pip-audit ran and found no issues."
    ],
    "npm_audit": self .run_npm_audit ()or [
        "✅ npm audit ran and found no issues."
    ]
}
...
""")
rendered_html =html_template .render (metadata =metadata,
summary =self .summary,
overall_security =overall_security,
security_data =security_data )
try :
with open (HTML_OUTPUT_FILE,
"w",
...
for lang_name,
dep_list in all_unused_deps .items ():
if dep_list :
f .write (f "  - **{lang_name}**:\n")
for dep in dep_list :
f .write (f "    - {dep}\n")
...
for func,
score in items .items ():
match =re .search (r"Score:\s*(\d+)",
score )
if match :
...
self .summary [
    category
].append (f "{file_path.name} (Path: {file_path.parent})")
try :
...
self .summary [
    "JS Functions"
].append (f "{func} (Found in: {file_path})")
for line in lines :
line_strip =line .strip ()
...
self .summary [
    "Security Issues"
].append (f "Sensitive Data Found in {file_path.name}: {line_strip}")
if "dangerouslySetInnerHTML"in line_strip or ".innerHTML"in line_strip :
self .summary [
    "Security Issues"
].append (f "Possible DOM-based XSS usage in {file_path.name}: {line_strip}")
if file_path .suffix in [
    ".js",
    ".jsx",
    ".tsx"
]and js_funcs :
calls =self .extract_javascript_invocations (lines,
js_funcs )
if calls :
for call in calls :
self .summary [
    "JS Invocations"
].append (f "{call} (Found in: {file_path})")
except Exception as e :
if self .verbose :
...
self .generate_security_report (manifest_findings,
eslint_issues,
stylelint_issues,
webext_issues,
pip_audit_findings,
...
"""
    ...
    """
parser =argparse .ArgumentParser (description ="Scan and analyze a code project.\n\n""This tool supports scanning Python, JavaScript, HTML, CSS files and more. ""It generates reports in Markdown and HTML (with interactive, emoji-rich sections).\n\n""Options:\n""  --gui    Launch a simple GUI for non-technical users.",
formatter_class =argparse .RawTextHelpFormatter )
parser .add_argument ("--dry-run",
action ="store_true",
help ="Perform a dry run without modifications")
parser .add_argument ("--verbose",
action ="store_true",
help ="Enable verbose output")
parser .add_argument ("--threshold",
type =int,
default =HIGH_COMPLEXITY_THRESHOLD_DEFAULT,
...
action ="store_true",
help ="Launch a simple GUI instead of command-line interface")
...
dry_run =False,
verbose =True,
threshold =HIGH_COMPLEXITY_THRESHOLD_DEFAULT )
...
tk .Label (root,
text ="Select Directory to Scan:").pack (padx =10,
pady =5 )
dir_var =tk .StringVar ()
tk .Entry (root,
textvariable =dir_var,
width =50 ).pack (padx =10,
pady =5 )
tk .Button (root,
text ="Browse...",
command =select_directory ).pack (padx =10,
pady =5 )
tk .Button (root,
text ="Start Scan",
command =start_scan ).pack (padx =10,
pady =10 )
root .mainloop ()
def main ()->None :
args =parse_args ()
